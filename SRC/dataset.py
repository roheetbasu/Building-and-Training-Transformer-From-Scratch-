...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #add sos, eos and pad to each sentences
        enc_num_padding_tokens = self.seq - 2 - len(enc_input_tokens) # we will add <s> and </s>
        dec_num_padding_tokens = self.seq - 1 -len(dec_input_tokens)
        ...
        return {
            ...
            "decoder_mask": (decoder_input != self.pad_token).unsqueeze(0).int() & self.causal_mask(decoder_input.size(0)), # (1, seq) & (1, seq, seq),
            "label": label,  # (seq)
            "src_text": src_text,
            "tgt_text": tgt_text,
        }
